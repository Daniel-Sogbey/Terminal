...
func (m *postgresDBRepo) SearchAvailabilityByDatesByRoomID(start, end time.Time, roomID int) (bool, error) {
	...
	// search date is exactly the same as existin reservation
	query := `
		select 
			count(id)
		from 
	...
	m.app.InfoLog.Println("Rows ", row)
	...
	m.app.InfoLog.Println("Rows nums", numRows)
...
func (m *postgresDBRepo) UpdateUser(user models.User) error {
	...
	stmt := `update 
				users set first_name = $1, last_name = $2, email = $3, access_level = $4, created_at = $5, updated_at = $6 
			where id = $7`
