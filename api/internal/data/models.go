...
func (t *Token) GenerateToken(userID int, ttl time.Duration) (*Token, error) {
	...
	token.Token = base32.StdEncoding.WithPadding(base32.NoPadding).EncodeToString(randomByte)
...
func (t *Token) Insert(token Token, user User) error {
	...
	stmt = `insert into tokens (user_id, email, token, token_hash, created_at, updated_at, expiry) values 
	($1,$2,$3,$4,$5,$6,$7)`

	_, err = db.ExecContext(ctx, stmt,
		token.UserId, token.Email, token.Token, token.TokenHash, time.Now(), time.Now(), token.Expiry)
